        ...
        if isinstance(properties, Mapping):
            for param_name, param_fields in properties.items():
                ...
                if isinstance(param_fields, Mapping):
                    ...
                    rendered += _render_extra_keys(param_fields, frozenset({"name", "type", "description"}))
                rendered += "\n</parameter>"
        rendered += _render_extra_keys(parameters, frozenset({"type", "properties"}))

    rendered += "\n</parameters>"
    rendered += _render_extra_keys(raw_tool, frozenset({"type", "name", "description", "parameters"}))
    rendered += "\n</function>"
    return rendered
...
class PrimeQwen3Renderer:
    ...
    def render(
        ...
        for loop_index, message in enumerate(loop_messages):
            ...
            if role == "assistant":
                ...
                self._render_tool(message, message_index, builder, opens_group=opens_group, closes_group=closes_group)
            else:
    ...
    def _render_assistant(self, message: Message, message_index: int, builder: RenderedTokenBuilder) -> None:
        content = self._content(message)
        builder.emit_special(self._im_start, message_index, is_sampled=False, is_content=False)

        if "reasoning_content" in message:
            builder.emit_text("assistant\n", message_index, is_sampled=False, is_content=False)
            builder.emit_special(self._think, message_index, is_sampled=True, is_content=True)
            reasoning = message.get("reasoning_content")
            if reasoning:
                builder.emit_text(str(reasoning).strip(), message_index, is_sampled=True, is_content=True)
            builder.emit_special(self._think_end, message_index, is_sampled=True, is_content=True)
            if content.strip():
    ...
    def _render_assistant_tool_calls(self, message: Message, message_index: int, builder: RenderedTokenBuilder) -> None:
        builder.emit_special(self._im_start, message_index, is_sampled=False, is_content=False)
        content = message.get("content")
        ...
        for tool_call_index, tool_call in enumerate(tool_calls):
            ...
            builder.emit_special(self._tool_call, message_index, is_sampled=True, is_content=True)
            ...
            call_text += "</function>\n"
            builder.emit_text(call_text, message_index, is_sampled=True, is_content=True)
            builder.emit_special(self._tool_call_end, message_index, is_sampled=True, is_content=True)
